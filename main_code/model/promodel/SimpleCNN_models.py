...
class DnCNN(nn.Module):
    def __init__(self, channels, num_of_layers=17, lip=1.0, no_bn=False,
        ...
        features = 64
        if lip > 0.0:
            sigmas = [pow(lip, 1.0/num_of_layers)
        ...
        if adaptive:
            ...
            assert len(
                sigmas) == num_of_layers, "Length of SN list uncompatible with num of layers."
        ...
        for i in range(1, num_of_layers-1):
            # conv layer
            layers.append(conv_layer(features, features, sigmas[i]))
