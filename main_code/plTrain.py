...
class cnnTrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        image = torch.from_numpy(np.transpose(np.asarray(Image.open(
            join(self.path, self.fileList[index]))), (2, 0, 1))).float()/255.0
...
class cnnTestDataset(Dataset):
    def __init__(self, path, snr):
        self.dataPath = path
        self.snr = snr
        self.fileList = listdir(self.dataPath)

    def __getitem__(self, index):
        image = torch.from_numpy(np.transpose(np.asarray(Image.open(
            join(self.dataPath, self.fileList[index]))), (2, 0, 1))).float()/255.0
        return image, torch.FloatTensor(image.size()).normal_(mean=0, std=self.snr/255.)
...
class plWrapper(pl.LightningModule):
    def __init__(self, cnnDepth=12,
        ...
        if self.hparams['type'] == 'conv':
            ...
            self.dncnn = UNetRes(in_nc=1, out_nc=1, act_mode='E',
                                 nb=self.hparams['numBlock'], bias=self.hparams['bias'])
    ...
    def training_step(self, batch, batch_idx):
        image, noise = batch
        imagePatch = rearrange(image.unfold(2, self.hparams.pSize, self.hparams.pStride).unfold(
            3, self.hparams.pSize, self.hparams.pStride), 'n c p1 p2 h w -> (n p1 p2) c h w')
        noisePatch = rearrange(noise.unfold(2, self.hparams.pSize, self.hparams.pStride).unfold(
            3, self.hparams.pSize, self.hparams.pStride), 'n c p1 p2 h w -> (n p1 p2) c h w')
        noisyImagePatch = imagePatch+noisePatch
        predNoisePatch = self(noisyImagePatch, create_graph=True, strict=True)

        loss = torch.mean(torch.pow(predNoisePatch-noisePatch, 2))
        with torch.no_grad():
            train_snr = compare_snr(imagePatch, noisyImagePatch-predNoisePatch)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        noisyImagePatch = imagePatch+noisePatch
        predNoisePatch = self(
            noisyImagePatch, create_graph=False, strict=False)
        valPSNR = 0
        valSSIM = 0
        with torch.no_grad():
            for i in range(imagePatch.shape[0]):
                valPSNR += peak_signal_noise_ratio(
                    imagePatch[i, 0, :, :].detach().cpu().numpy(), torch.clamp(noisyImagePatch[i, 0, :, :]-predNoisePatch[i, 0, :, :], min=0, max=1).detach().cpu().numpy(), data_range=1)
                valSSIM += structural_similarity(imagePatch[i, 0, :, :].detach().cpu(
                ).numpy(), torch.clamp(noisyImagePatch[i, 0, :, :]-predNoisePatch[i, 0, :, :], min=0, max=1).detach().cpu().numpy(), data_range=1)
        self.log("val_psnr", valPSNR/imagePatch.shape[0], on_step=False,
                 on_epoch=True, prog_bar=False, logger=True)
        self.log("val_ssim", valSSIM/imagePatch.shape[0], on_step=False,
                 on_epoch=True, prog_bar=False, logger=True)
        return {'psnr': valPSNR/imagePatch.shape[0], 'ssim': valSSIM/imagePatch.shape[0]}
